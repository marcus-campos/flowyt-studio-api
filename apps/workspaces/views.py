...
from utils.redis import redis_monitor, redis_workspace
...
class WorkspaceViewSet(viewsets.ModelViewSet):
    ...
    def _perform_redis(self, workspace):
        ...
        release = Release.objects.filter(workspace=workspace, published=True).first()
        if release:
            ...
            for environment in workspace_release.environmentrelease_set.all():
                workspace_env = slugify("{0}-{1}".format(workspace_name, environment.name))
...
class MonitorViewSet(viewsets.ModelViewSet):
    ...
    def _perform_redis(self, monitor_id, destroy=False):
        try:
            ...
            if not destroy:
                redis_monitor.set(
                    "{0}.{1}".format(subdomain, workspace), json.dumps(monitor.monitor_variables)
                )
            else:
...
class ReleasePublishView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        Release.objects.filter(pk=serializer.validated_data["release"].id).update(published=True)
